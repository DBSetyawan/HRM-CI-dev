$(document).ready(function() {
   var xin_table = $('#xin_table').dataTable({
	...
	var xin_my_team_table = $('#xin_my_team_table').dataTable({
		"bDestroy": true,
		"ajax": {
			url : site_url+"timesheet/my_team_slip_list/",
			type : 'GET'
		},
		"fnDrawCallback": function(settings){
		$('[data-toggle="tooltip"]').tooltip();          
	...
	jQuery("#aj_company").change(function(){
		jQuery.get(base_url+"/get_slip_employees/"+jQuery(this).val(), function(data, status){
	...
	$("#ihr_report").submit(function(e){
		...
		e.preventDefault();
			 var xin_table2 = $('#xin_table').dataTable({
				"bDestroy": true,
				"ajax": {
					url : site_url+"slip/slip_list/?ihr=true&company_id="+$('#aj_companyf').val()+"&employee_id="+$('#employee_id').val()+"&status="+$('#status').val(),
					type : 'GET'
				},
				"fnDrawCallback": function(settings){
					$('[data-toggle="tooltip"]').tooltip();          
