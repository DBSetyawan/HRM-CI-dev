...
var xin_discussion_table = $('#xin_discussion_table').dataTable({
	"bDestroy": true,
	"ajax": {
		url : site_url+"quoted_projects/discussion_list/"+$('#tproject_id').val(),
		type : 'GET'
	},
	"iDisplayLength": 25,
...
var xin_attachment_table = $('#xin_attachment_table').dataTable({
	"bDestroy": true,
	"ajax": {
		url : site_url+"quoted_projects/attachment_list/"+$('#f_project_id').val(),
		type : 'GET'
	},
	"fnDrawCallback": function(settings){
	$('[data-toggle="tooltip"]').tooltip();          
...
$('.edit-modal-timelog-data').on('show.bs.modal', function (event) {
	...
	var modal = $(this);
	$.ajax({
	url : site_url+"quoted_projects/read_timelog_record/",
	type: "GET",
...
var xin_timelogs_table = $('#xin_timelogs_table').dataTable({
	"bDestroy": true,
	"ajax": {
		url : site_url+"quoted_projects/project_timelogs_list/"+$('#f_project_id').val(),
		type : 'GET'
	},
	"fnDrawCallback": function(settings){
	$('[data-toggle="tooltip"]').tooltip();          
...
$( document ).on( "click", ".fidelete", function() {
	...
	$('#delete_record_f').attr('action',site_url+'quoted_projects/attachment_delete/'+$(this).data('record-id'));
});
