...
class Xin_model extends CI_Model {
	...
	public function select_module_class($mClass,$mMethod) {
		...
		if($mClass=='dashboard') {
			...
			$arr['emp_dashboard_active'] = 'active';
			...
			$arr['quote_timelogs_active'] = 'active';
		...
		else if($mClass=='jadwals' && $mMethod=='calendar') {
			$arr['hr_ecalendar_active'] = 'active';
			$arr['hr_jadwals_open'] = 'active';
			return $arr;
		}
			...
			$arr['hr_events_open'] = 'active';
			return $arr;
		} else if($mClass=='jadwals') {
			$arr['hr_jadwals_active'] = 'active';
			$arr['hr_jadwals_open'] = 'active';
			return $arr;
...
ORDER BY `expiry_date`");
	...
	public function get_company_employees($company_id,$limit=NULL,$start=NULL,$search=NULL,$order=[]) {
		
		$sql = 'SELECT * FROM xin_employees WHERE company_id = ?';
		if(!empty($search)){
			$sql .= " and ( lower(first_name) like '%".$search."%' or "."lower(last_name) like '%".$search."%' "." or lower(employee_id) like '%".$search."%' ) ";
		}
		if(empty($order)){
	...
	public function get_employee_jadwals_count($id) {
