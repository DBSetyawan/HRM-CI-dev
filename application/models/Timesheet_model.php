<?php
	...
	// Manager
	public function get_manager_leaves($role_id) {
	
		$sql = 'SELECT * FROM xin_leave_applications WHERE role_id = ?';
		$binds = array($company_id);
		$query = $this->db->query($sql, $binds);
		return $query;
	}
	...
	public function read_leave_type_information($id) {
		...
		$sql = 'SELECT * FROM masterijin WHERE Jenis = ? limit 1';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function get_no_ref(){
		$db2 = $this->load->database('db_payroll', TRUE);
        $cd = $db2->query("SELECT MAX(RIGHT(`No Ijin`,9)) AS kd_max FROM ijin");
        $kd = "";
        if($cd->num_rows()>0){
    ...
    public function sisa_cuti($id){
		$db2 = $this->load->database('db_payroll', TRUE);
        $sql = "SELECT m.Begda, m.Endda, m.Nip, e.Nama, m.Quota, m.Diambil, (m.Quota - m.Diambil)as sisa FROM mastercuti m left join masteremployee e on m.nip=e.nip WHERE (m.Begda >= CONCAT(year(CURRENT_DATE()),'-01-01') and m.Endda >= CURRENT_DATE()) AND m.Nip = ? limit 1";
        $binds = array($id);
		$query = $db2->query($sql, $binds);
    ...
    //JUMLAH CUTI YANG SUDAH DIAMBIL PER BULAN
    public function banyak_ambil_cuti_perbulan($id, $month){
		$db2 = $this->load->database('db_payroll', TRUE);
        $sql = "SELECT Nip, `Jenis Ijin`, month(`Tgl Ijin`)Bulan, year(`Tgl Ijin`)Tahun, count(`Tgl Ijin`)as jum_cuti FROM ijin i
					  where Nip = ? and year(`Tgl Ijin`) = year(CURRENT_DATE()) and month(`Tgl Ijin`) = ?
					  group by Nip, month(`Tgl Ijin`),year(`Tgl Ijin`)
					  order by `No Ijin` asc";
    ...
    public function cuti_book($id){
        $sql = "SELECT employee_id, sum(jum) as jum_book FROM(
					SELECT employee_id, datediff(to_date, from_date)+1 as jum FROM xin_leave_applications x
					  where employee_id = ?
					    and leave_type_id='C'
					    and status !=3
					    and approved_mgr != 3
					    ...
					    and year(to_date)=year(CURRENT_DATE())
					 group by leave_id)a limit 1";
        $binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function check_user_attendance($user_id='') {
		...
		if(empty($user_id)){
			...
			$binds = array($user_id,$today_date);
		}
		$query = $this->db->query($sql, $binds);
