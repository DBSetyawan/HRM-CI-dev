...
class Job_candidates extends MY_Controller {
    ...
    public function candidate_list() {
		...
		if(in_array('295',$role_resources_ids)) { // delete
			...
			$edit_status = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_change_status').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".add-modal-data" data-application_id="'. $r->application_id . '"><i class="fa fa-pencil-square-o"></i></button></span>';
		} else {
			$delete = '';
			$edit_status = '';
		}
		$iticket_code = $r->full_name.'<br><small class="text-muted"><i>'.$r->email.'<i></i></i></small>';
		$cover_letter = '<a><button class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-application_id="'. $r->application_id . '">'.$this->lang->line('xin_view').' '.$this->lang->line('xin_jobs_cover_letter').'</button></a>';
	...
	public function applicants_list() {
		...
		$iticket_code = $r->full_name.'<br><small class="text-muted"><i>'.$r->email.'<i></i></i></small>';
		$cover_letter = '<a><button class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-application_id="'. $r->application_id . '" data-user_id="'. $r->user_id . '">'.$this->lang->line('xin_view').' '.$this->lang->line('xin_jobs_cover_letter').'</button></a>';
		$combhr = $download.$edit_status.$delete;
