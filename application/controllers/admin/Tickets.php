...
class Tickets extends MY_Controller {
    ...
    public function ticket_list(){
		...
		if($r->ticket_status==1):
			$status = '<span class="badge bg-orange">'.$this->lang->line('xin_open').'</span> '.$xpired;
		else:
			$status = '<span class="badge bg-green">'.$this->lang->line('xin_closed').'</span> '.$xpired;
		endif;
		 ...
		 if(!is_null($created_by)){
			...
			if($created_by[0]->profile_picture!='' && $created_by[0]->profile_picture!='no file') {
				$eol = '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" title="'.$ticket_created_by.'"><span class="avatar box-32"><img src="'.base_url().'uploads/profile/'.$created_by[0]->profile_picture.'" class="user-image-hr" alt=""></span></a>';
				} else {
				...
				$eol = '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" title="'.$ticket_created_by.'"><span class="avatar box-32"><img src="'.$ede_file.'" class="user-image-hr" alt=""></span></a>';
				}
		 ...
		 // ticket attachment
		 if($r->ticket_image!='0'){
			 $timage = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download').'"><a href="'.site_url().'admin/download?type=ticket&filename='.$r->ticket_image.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-download"></span></button></a></span>';
		 } else {
			 $timage = '';
		 }
		...
		$ipriority = $priority.'<br><small class="text-muted"><i>'.$end_date.'<i></i></i></small><br><small class="text-muted"><i>'.$xpired.'<i></i></i></small>';
		$data[] = array(
			$combhr,
			$iticket_code,
			$iemployee_name,
	  ...
	  public function attachment_list()
     {
		...
		$delopt = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete-file" data-toggle="modal" data-target=".delete-modal-file" data-record-id="'. $r->ticket_attachment_id . '"><i class="fa fa-trash-o"></i></button></span>';	
		} else {
			$delopt = '';
		}
		
		$data[] = array('<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download').'"><a href="'.site_url().'admin/download?type=ticket&filename='.$r->attachment_file.'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"><i class="fa fa-download"></i></button></a></span>'.$delopt,
			$r->file_title,
			$r->file_description,
			$r->created_at
		);
	...
	public function details()
     {
		...
		$this->Tickets_model->update_record($edata,$id);
