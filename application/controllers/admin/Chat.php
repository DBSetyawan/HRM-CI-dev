   ...
   /*Function to set JSON output*/
			  ...
			  <div class="chats" id="chatbox">';
				foreach($this->Chat_model->get_messages() as $msgs){
					...
					if($session['user_id']!=$msgs->from_id){
                      ...
                      <div>
                        <img src="'.$de_file.'" class="direct-chat-img" alt="">
                      </div>
					  ...
					  <div>
						<img src="'.$fde_file.'" class="direct-chat-img" alt="">
					  </div>
				...
				foreach($this->Chat_model->get_messages() as $msgs){
					...
					if($session['user_id']!=$msgs->from_id){
                      ...
                      <div class="direct-chat-info clearfix">
                        <span class="direct-chat-name pull-left">'.$tofname.'</span>
                        <span class="direct-chat-timestamp pull-right time-right">'.$last_chat_date.'</span>
                      </div>
                      <img class="direct-chat-img" src="'.$de_file.'" alt="">
                      ...
                      <div class="direct-chat-info clearfix">
                        ...
                        <span class="direct-chat-timestamp pull-left time-left">'.$last_chat_date.'</span>
                      </div>
                      <img class="direct-chat-img" src="'.$fde_file.'" alt="">
	...
	public function refresh_chat_users() {
		...
		if ($active_employees->is_logged_in == 0):
			$bgm = 'avatar-away';
			$bgmTxt = $this->lang->line('xin_chatoffline_title');
        else:
			if($active_employees->online_status==1):
         		$bgm = 'avatar-online';
				$bgmTxt = $this->lang->line('xin_chatonline_title');
        ...
        echo '<li class="avatfar avatar-md '.$bgm.'"><button class="all-users list-group-item list-group-item-action media no-border" id="set_box_'.$active_employees->user_id.'" data-from-id="'.$session['user_id'].'" data-to-id="'.$active_employees->user_id.'" data-toggle="modal" data-target=".chatbox-single"><a href="#">';
		
          	if($active_employees->profile_picture!='' && $active_employees->profile_picture!='no file') {
            echo '<img class="direct-chat-img" src="'.base_url()."uploads/profile/".$active_employees->profile_picture.'" alt=""> <i></i>';
             } else {
            ...
            echo '<img class="direct-chat-img" src="'.$de_file.'" alt=""> <i></i>';
              } 
			...
			echo '<div class="menu-info">
                		<h4 class="control-sidebar-subheading">'.$fname.' <span class="label text-green pull-right" style="margin-right: 10px;">'.$last_chat_date.'</span></h4><p>'.substr($message_content,0,8).'...';
				if($unread_msgs > 0) {
					echo '<span class="badge bg-aqua pull-right">'.$unread_msgs.'</span>';
				} else {
				} echo '</p>
              </div></a></button> 
					</li>';
