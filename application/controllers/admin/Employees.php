...
class Employees extends MY_Controller {
	...
	public function dialog_emp_overtime() {
	 ...
	 public function designation() {
		...
		$is_departement = $this->uri->segment(5);
		if(!empty($is_departement)){
			$data['is_departement'] = 1;	
		}
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
	...
	public function basic_info() {
		...
		*/
		else if (!empty($this->input->post('email')) && !filter_var($this->input->post('email'), FILTER_VALIDATE_EMAIL)) {
			$Return['error'] = $this->lang->line('xin_employee_error_invalid_email');
		} else if($this->input->post('company_id')==='') {
			 $Return['error'] = $this->lang->line('error_company_field');
