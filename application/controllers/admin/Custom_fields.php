		...
		$data = array();
          ...
          foreach($custom_fields->result() as $r) {
			  ...
			  if(in_array('395',$role_resources_ids)) { //edit
				$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target="#edit-modal-data"  data-custom_field_id="'. $r->custom_field_id . '"><span class="fa fa-pencil"></span></button></span></span>';
			} else {
				$edit = '';
			}
			if(in_array('396',$role_resources_ids)) { // delete
				$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->custom_field_id . '"><span class="fa fa-trash"></span></button></span>';
			} else {
				$delete = '';
			}
			if($r->validation == 0){
