...
class Timesheetreport_wfh extends MY_Controller {
	 ...
	 public function __construct() {
		...
		$this->load->model("Timesheetreportwfh_model");
	}
	...
	public function index(){
		...
		if(in_array('423',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['hadir'] = $this->Timesheetreportwfh_model->get_hadir();
				$data['absen'] = $this->Timesheetreportwfh_model->get_absen();
				$data['luarkota'] = $this->Timesheetreportwfh_model->get_luarkota();
				$data['dalamkota'] = $this->Timesheetreportwfh_model->get_dalamkota();
				$data['dangerarea'] = $this->Timesheetreportwfh_model->get_dangerarea();
	...
	public function history_pegawai(){
		...
		if(in_array('423',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['history'] = $this->Timesheetreportwfh_model->get_history($attendance_date,$employee_id);
	...
	public function haslogin(){
		...
		if(in_array('423',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['absen'] = $this->Timesheetreportwfh_model->get_hadir_rpt();
    ...
    public function get_login_rpt(){
		...
		$data_result = $this->Timesheetreportwfh_model->get_hadir_rpt($attendance_date);
		$data = [];
		foreach ($data_result as $key => $value) {
			...
			$data[] = [$nama,$value->contact_no, $attendance_date, $value->first_log,$value->last_log,$detail];
		}
		$output = array(
		   "draw" => $draw,
    ...
    public function get_notlogin(){
		...
		$data_result = $this->Timesheetreportwfh_model->get_absen($attendance_date);
		$data = [];
		foreach ($data_result as $key => $value) {
			$nama = $value->first_name.(!empty($value->last_name)? " ".$value->last_name : "");
			$data[] = ["karyawan" => $nama,"employee_id" => $value->employee_id,"contact_no" => $value->contact_no,"date" => $attendance_date,"status" => "Tidak Login"];
		}
		$output = array(
		   "draw" => $draw,
			 "recordsTotal" => count($data),
    ...
    public function get_dangerarea_list(){
		...
		$data_result = $this->Timesheetreportwfh_model->get_dangerarea($attendance_date);
		$data = [];
		foreach ($data_result as $key => $value) {
			$nama = $value->first_name.(!empty($value->last_name)? " ".$value->last_name : "");
			$detail = "<a href='".base_url()."admin/timesheetreport/history_pegawai?date=".$attendance_date."&employee_id=".$value->user_id."' target='_blank'>Detail</a>";
    ...
    public function get_dalamkota_list(){
		...
		$data_result = $this->Timesheetreportwfh_model->get_dalamkota($attendance_date);
		$data = [];
		foreach ($data_result as $key => $value) {
			$nama = $value->first_name.(!empty($value->last_name)? " ".$value->last_name : "");
			$detail = "<a href='".base_url()."admin/timesheetreport/history_pegawai?date=".$attendance_date."&employee_id=".$value->user_id."' target='_blank'>Detail</a>";
    ...
    public function get_luarkota_list(){
		...
		$data_result = $this->Timesheetreportwfh_model->get_luarkota($attendance_date);
		$data = [];
		foreach ($data_result as $key => $value) {
			$nama = $value->first_name.(!empty($value->last_name)? " ".$value->last_name : "");
			$detail = "<a href='".base_url()."admin/timesheetreport/history_pegawai?date=".$attendance_date."&employee_id=".$value->user_id."' target='_blank'>Detail</a>";
    ...
    public function get_history_list(){
		...
		$data_result = $this->Timesheetreportwfh_model->get_history($attendance_date,$employee_id);
		$data = [];
		foreach ($data_result as $key => $value) {
			$nama = $value->first_name.(!empty($value->last_name)? " ".$value->last_name : "");
			$detail = "<a href='".base_url()."admin/timesheetreport/history_pegawai?date=".$attendance_date."&employee_id=".$value->user_id."' target='_blank'>Detail</a>";
