...
class Quoted_Projects extends MY_Controller {
	...
	<div class="progress progress-xs"><div class="progress-bar '.$progress_class.' progress-bar-striped" role="progressbar" aria-valuenow="'.$r->project_progress.'" aria-valuemin="0" aria-valuemax="100" style="width: '.$r->project_progress.'%"></div></div></p>';
			...
			$project_summary = '<a href="'.site_url().'admin/quoted_projects/detail/'.$r->project_id . '">'.$r->title.'</a><br><small>'.$this->lang->line('xin_project_client').': '.$client_name.'</small><br><small>'.$this->lang->line('xin_estimate_hrs').': '.$r->estimate_hrs.'</small>';
			
			// progress
			$project_progress = $pbar.$status;
			$project_no = '<a href="'.site_url().'admin/quoted_projects/detail/'.$r->project_id . '">'.$r->project_no.'</a>';
			$combhr = $edit.$delete;
			$data[] = array(
				$combhr,
				$project_no,
